...
app.post('/api/widget/onboard', async (req, res) => {
  try {
    ...
    console.error('Error onboarding visitor:', error);
...
`;
    ...
    if (groqApiKey) {
      try {
        ...
        console.error('Error communicating with Groq API:', groqErr);
    ...
    console.error('Error handling chat exchange:', error);
...
app.get('*', (req, res, next) => {
  ...
  res.sendFile(path.join(frontendBuildPath, 'index.html'), err => {
    if (err) {
      ...
      res.status(200).send('Varta Assistant Backend running. Frontend dist not detected. Use npm run dev for dev environments.');
    }
  });
});
...
app.listen(PORT, () => {
  console.log(`=======================================================`);
  console.log(` VARTA ASSISTANT IS LIVE ON PORT ${PORT}`);
